...
### * 如何实现深度克隆？如何实现一个浅复制？
  ...
  * 浅复制
    其实浅复制根深度复制基本上差不多，只是在第一次遍历的时候，不需要判定对象值的类型进行下一步的递归， 直接赋值即可。 数组的浅复制也可以直接用 slice方法 既可实现。
### * 如何实现 ```ajax``` 的拦截处理?
...
### * 什么是 ```XSS``` 和 ```CSRF```？ 如何预防？
  * XSS 是跨站脚本攻击 ，指利用网站没有对用户提交数据进行转义处理或过滤不足导致恶意代码嵌入到了web页面中去，使的别的用户访问都会执行这段代码。从而盗取用户资料，获悉隐私信息等危害。形成的主要原因就过于信任客户端提交的数据。 
  解决办法： 不信任任何前端提交的数据，对提交的数据先进行相应的过滤和转码处理。
  * CSRF 跨站请求伪造攻击。  是夹持用户在已经登陆的web应用上执行非本意的操作。攻击者一般是通过存储的cookie获取到用户的登陆态，在某一个页面中产生一个隐藏的链接，像服务端提交数据，进行攻击。
  防御办法： 验证referer字段； 为请求添加token； 使用验证码
